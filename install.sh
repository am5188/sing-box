...
author=am5188
# github=https://github.com/am5188/sing-box
...
# curl installed or none
command -v curl >/dev/null 2>&1 || err "未检测到 curl，请先安装 curl。"
is_wget=1
...
# curl download helper (保持旧接口)
_wget() {
    local url=""
    local output=""
    local retry=0
    local continue_download=0
    local stdout=0
    local ip_mode=""
    ...
    local args=(--fail --location --proto '=https' --tlsv1.2)
    [[ $retry -gt 0 ]] && args+=(--retry "$retry" --retry-all-errors)
    [[ $continue_download -eq 1 ]] && args+=(-C -)
    [[ $ip_mode == "-4" ]] && args+=(-4)
    [[ $ip_mode == "-6" ]] && args+=(-6)
    ...
    if [[ $stdout -eq 1 ]]; then
        ...
        [[ -z $output ]] && output="${TMPDIR:-/tmp}/sbm-download-$RANDOM"
        args+=(-o "$output")
        curl "${args[@]}" "$url"
    fi
}
...
main() {
    ...
    if [[ ! $is_core_file && -f $is_core_ok ]]; then
        ...
        if [[ -t 0 ]]; then
            ...
            read -rp "确认继续安装? [y/N]: " confirm_sha </dev/tty || confirm_sha=""
        fi
    ...
    if [[ $local_install ]]; then
        ...
        mkdir -p $tmpdir/shpkg
        tar zxf $is_sh_ok -C $tmpdir/shpkg
        local pkg_dir
        pkg_dir=$(find $tmpdir/shpkg -mindepth 1 -maxdepth 1 -type d | head -n1)
        [[ $pkg_dir ]] || {
            msg err "脚本文件解压失败."
            exit_and_del_tmpdir
        }
        cp -rf $pkg_dir/* $is_sh_dir
    fi
